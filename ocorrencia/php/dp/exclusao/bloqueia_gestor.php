...
<body>
	...
	<div class="">
		<form class="form_cadastro_unidade" action="bloqueando_gestor.php" method="post" accept-charset="utf-8">
		<fieldset>
			<legend>Bloqueio de usuário</legend>
				<?php
					...
					$usuario_sessao = isset($_SESSION['usuario'])?$_SESSION['usuario']:'';
 					...
 					while ($resultado2 = mysqli_fetch_array($select_gestor)) {
							echo "<h5>Você está logado como $resultado2[0]!</h5><br><p>Preste atenção você está prestes a bloquear um usuário.</p>";
					}

					
					#Em baixo são inputs e selects de formulário

					echo "<label><i class='fa fa-address-card-o' aria-hidden='true'></i>Digite o Usuário: <input type='text' style='width:400px ' name='usuario' placeholder='Digite o usuário que deseja bloquear . . .' required='' ></label> ";
					
				?>
		</fieldset>
				<button type="submit" class="btn btn_azul"><i class="fa fa-user-times" aria-hidden="true"></i> Bloquear</button>
		</form><br>
	</div><!-- Div formulario -->
	...
	<script>
		$(document).ready(function(){
			$('.btn').click(function(){
				$('.btn').text('Bloqueando, por favor aguarde...').addClass('btn_azul_efeito');
			});
		});
	</script>
</body>
