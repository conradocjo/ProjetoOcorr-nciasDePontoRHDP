...
<body>
	...
	<div class="">
		<form class="form_cadastro_unidade" action="excluindo_setor.php" method="post" accept-charset="utf-8">
		<fieldset>
			<legend>Exclusão de Setor</legend>
			<?php
				...
				$usuario_sessao = isset($_SESSION['usuario'])?$_SESSION['usuario']:'';
 				...
 				while ($resultado2 = mysqli_fetch_array($select_gestor)) {
					echo "<h5>Você está logado como $resultado2[0]!</h5><p>Atenção você está prestes a apagar um setor.</p>";
				}
	...
	<script>
		$(document).ready(function(){
			$('.btn').click(function(){
				$('.btn').text('Excluindo, por favor aguarde...').addClass('btn_azul_efeito');
			});
		});
	</script>
</body>
