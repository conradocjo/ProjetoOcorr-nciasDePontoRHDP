...
<body>
	...
	<div class="">
		<form class="form_cadastro_unidade" action="corrigindo_setor.php" method="post" accept-charset="utf-8">
		<fieldset>
			...
			<?php
				...
				$usuario_sessao = isset($_SESSION['usuario'])?$_SESSION['usuario']:'';
 				...
 				$select_setor_nome = "SELECT id , nome FROM setor";
 				$select_gestor = mysqli_query($conexao, $sql_select_nome_gestor);
 				$select_setor = mysqli_query($conexao, $select_setor_nome);
 				while ($resultado2 = mysqli_fetch_array($select_gestor)) {
					echo "<h5>Você está logado como $resultado2[0]!</h5>";
				}

				echo"<label><i class='fa fa-users' aria-hidden='true'></i> Selecione o setor que deseja alterar:  <select name='id_setor' >";
							while ($result1 = mysqli_fetch_array($select_setor)) {
								echo"
									<option value='$result1[0]'>$result1[1]</option>";
							}
				...
				<input type='text'placeholder='Digite o nome do setor . . .' style='width: 350px ' name='setor'> ";
					#Select Unidade

				#input para ocorrencia de rastreamento
				echo "<input type='hidden' name='rastro' value='$usuario_sessao'> ";
			?>
		</fieldset><br>
				<button type="submit" class="btn btn_azul"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Corrigir</button>
		</form><br>
	</div><!-- Div formulario -->
