...
<body>
	...
	<?php require_once"../../../menu.php";?>
		...
		<?php
			...
			$conexao = conecta();
			#manipula post
			$senha_empregado = isset($_POST["senha"])?$_POST["senha"]:"";
			...
			$insert_dados = "UPDATE empregado SET senha =  MD5('$senha_empregado'), usuario_rastro_atualizacao = '$usuario_sessao' WHERE matricula = '$matricula' ";
			...
			if ($validacao1 = mysqli_num_rows($testa_dados)){
				insert($insert_dados);
				echo "<div class='cadastro_existente'><h1>Senha do empregado de matrícula $matricula alterado com sucesso.</h1></div>";
			}else{
				if($senha==""){
					echo "<div class='cadastro_existente'><h1>Dados não foram preenchidos. </h1></div>";
				}else{
					
					echo "<div class='cadastro_existente'><h1>empregado de matrícula $matricula não encontrado.</h1></div>";
				}
			}
