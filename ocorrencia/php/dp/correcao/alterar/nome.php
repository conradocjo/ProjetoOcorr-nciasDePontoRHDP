...
<body>
	...
	<?php require_once"../../../menu.php";?>
		...
		<?php
			...
			#querys
			$insert_dados = "UPDATE usuario_gestor SET nome = '$nome', usuario_rastro_atualizacao = '$usuario_sessao' WHERE usuario = '$user_post' ";
			...
			if ($validacao1 = mysqli_num_rows($testa_dados)){
				insert($insert_dados);
				echo "<div class='cadastro_existente'><h1>Usuário $nome alterado com sucesso.</h1></div>";
			}else{
				if($setor==""){
					echo "<div class='dado_existente'><h1>Dados não foram preenchidos. </h1></div>";
				}else{
					
					echo "<div class='cadastro_realizado'<h1>Usuário $nome não encontrado.</h1></div>";
				}
			}
