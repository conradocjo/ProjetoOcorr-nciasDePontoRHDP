...
<body>
	...
	<?php require_once"../../../menu.php";?>
		...
		<?php
			...
			#querys
			$insert_dados = "UPDATE empregado SET nome = '$nome', usuario_rastro_atualizacao = '$usuario_sessao' WHERE matricula = '$matricula' ";
			...
			if ($validacao1 = mysqli_num_rows($testa_dados)){
				insert($insert_dados);
				echo "<div class='cadastro_existente'><h1>Empregado com a matricula $matricula alterado com sucesso.</h1></div>";
			}else{
				if($setor==""){
					echo "<div class='cadastro_existente'><h1>Dados não foram preenchidos.</h1></div>";
				}else{
					
					echo "<div class='cadastro_existente'><h1>Empregado com a matricula $matricula não encontrado.</h1></div>";
				}
			}
